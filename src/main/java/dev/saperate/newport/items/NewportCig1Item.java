...
public class NewportCig1Item extends Item {
    public NewportCig1Item(Settings settings) {
        super(settings);
    }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        if(!user.isSubmergedInWater()){
            user.setStackInHand(hand, NewportItems.NEWPORT_CIG_ITEM_2.getDefaultStack());
